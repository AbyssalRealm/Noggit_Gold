...
struct ClearingOptions
{
  ...
  bool textures_below_threshold = false;
  float alpha_threshold = 1.0f;
  bool texture_flags = false;
  bool liquids = false;
  bool m2s = false;
  bool wmos = false;
  bool shadows = false;
  bool vertex_colors = false;
  bool impassible_flag = false;
  bool holes = false;
};
...
public:
  ...
  std::optional<std::pair<float, glm::vec3>> intersectLiquid(math::ray const& ray);
...
public:
  ...
  // Red-native Clear/Eraser tool entry points. Mutations register against the
  // current Action so Ctrl+Z/redo restore the cleared data.
  void clearOnChunks(glm::vec3 const& pos, float radius, ClearingOptions const& options);
  void clearOnTiles(glm::vec3 const& pos, float radius, ClearingOptions const& options);
...
private:
  bool clearOnChunk(MapChunk* chunk, ClearingOptions const& options);
