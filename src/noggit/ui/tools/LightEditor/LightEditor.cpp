...
// 	}
	...
	addNewSkyButton->setEnabled(false);
	addNewSkyButton->setToolTip("Disabled in the WotLK safety baseline: duplicate currently shares LightParams/data records.");
	...
	QPushButton* portToSkyButton = new QPushButton("Port to Light", this);
	portToSkyButton->setIcon(Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::running));
	portToSkyButton->setToolTip("Move the camera to the selected local light center.");
	...
	connect(_active_lights_tree, &QListWidget::itemClicked, this, [=](QListWidgetItem* item)
		{
	...
	connect(_light_tree, &QListWidget::currentItemChanged, this, [=](QListWidgetItem* current, QListWidgetItem*)
		{
			if (!current)
				return;

			unsigned int selected_light_id = current->data(Qt::UserRole + 1).toUInt();
			Sky* sky = _map_view->getWorld()->renderer()->skies()->findSkyById(selected_light_id);
			if (sky)
				loadSelectSky(sky);
		});
	...
	connect(portToSkyButton, &QPushButton::clicked, [=]() {
			...
			if (!sky)
			{
				...
				(nullptr
					, "No light selected"
					, "Select a light from Current Map Lights first."
					, QMessageBox::Ok
				);
				return;
			}
			...
			if (sky->global)
			{
				QMessageBox::warning
				(nullptr
					, "Cannot port to Global Light"
					, "Global Lights have no world-space center to port to. Select a local light instead."
					, QMessageBox::Ok
				);
				return;
			}
			...
			loadSelectSky(sky);
			_map_view->move_camera_with_auto_height(sky->pos);
			updateActiveLights();
			updateLightningInfo();
	...
	connect(save_current_sky_button, &QPushButton::clicked, [=]() {
		...
		const bool fallback_global = curr_sky->global
			&& _map_view->getWorld()->renderer()->skies()->using_fallback_global;
		if (fallback_global)
		{
			QMessageBox::warning
			(nullptr
				, "Save blocked"
				, "This map is using the fallback Global Light. Saving it would modify the shared fallback Light record."
				, QMessageBox::Ok
			);
			return;
		}
	...
	connect(glow_slider, &QSlider::valueChanged, [&](int v) {
		get_selected_sky()->getParam(param_combobox->currentIndex()).value()->set_glow(v / 100.0f);
		updateLightning();
		});
	...
	connect(shallow_water_alpha_slider, &QSlider::valueChanged, [&](int v) {
		get_selected_sky()->getParam(param_combobox->currentIndex()).value()->set_river_shallow_alpha(v / 100.0f);
		updateLightning();
		});

	connect(deep_water_alpha_slider, &QSlider::valueChanged, [&](int v) {
		get_selected_sky()->getParam(param_combobox->currentIndex()).value()->set_river_deep_alpha(v / 100.0f);
		updateLightning();
		});

	connect(shallow_ocean_alpha_slider, &QSlider::valueChanged, [&](int v) {
		get_selected_sky()->getParam(param_combobox->currentIndex()).value()->set_ocean_shallow_alpha(v / 100.0f);
		updateLightning();
		});

	connect(deep_ocean_alpha_slider, &QSlider::valueChanged, [&](int v) {
		get_selected_sky()->getParam(param_combobox->currentIndex()).value()->set_ocean_deep_alpha(v / 100.0f);
		updateLightning();
		});
...
void LightEditor::loadSelectSky(Sky* _curr_sky)
{
	...
	for (Sky& sky : _map_view->getWorld()->renderer()->skies()->skies)
	{
		sky.set_selected(false);
	}
	_curr_sky->set_selected(true);

	{
		QSignalBlocker const tree_blocker(_light_tree);
		for (int i = 0; i < _light_tree->count(); ++i)
		{
			QListWidgetItem* item = _light_tree->item(i);
			if (item && item->data(Qt::UserRole + 1).toUInt() == _curr_sky->getId())
			{
				_light_tree->setCurrentItem(item);
				_light_tree->scrollToItem(item);
				break;
	...
	const bool fallback_global = _curr_sky->global
		&& _map_view->getWorld()->renderer()->skies()->using_fallback_global;
	save_current_sky_button->setEnabled(!fallback_global);
	if (fallback_global)
		save_current_sky_button->setToolTip("Save blocked: this map is using the shared fallback Global Light.");
	else
		save_current_sky_button->setToolTip(QString());
...
Sky* Noggit::Ui::Tools::LightEditor::get_tree_selected_sky() const
{
	auto const& selected_items = _light_tree->selectedItems();
	...
	unsigned int selected_light_id = selected_items.back()->data(Qt::UserRole + 1).toUInt();
	return _map_view->getWorld()->renderer()->skies()->findSkyById(selected_light_id);
}
