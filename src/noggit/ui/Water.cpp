...
namespace Noggit
{
  namespace Ui
  {
      ...
      , tile(0, 0)
    {
      ...
      auto* follow_terrain = new CheckBox("Follow terrain height", &_follow_terrain, this);
      follow_terrain->setToolTip("Shape painted liquid vertices to the terrain with a 1-unit height offset. This takes priority over Lock and Angled mode.");
      layout->addRow(follow_terrain);
      ...
      auto* liquid_intersection = new CheckBox("Cursor intersect liquids", &_cursor_intersect_liquids, this);
      liquid_intersection->setToolTip("Use the visible liquid surface as the Water Tool cursor height when it is closer than terrain.");
      layout->addRow(liquid_intersection);
    ...
    {
      bool const use_stroke_height = !_follow_terrain.get() && !_locked.get() && _stroke_active;
      bool const use_reference_height = _locked.get() || use_stroke_height;
      glm::vec3 const& reference_position = _locked.get() ? _lock_pos : _stroke_pos;
    ...
    void water::beginPaintStroke(glm::vec3 const& cursor_pos)
    {
      _stroke_pos = cursor_pos;
      _stroke_pos.y += 1.0f;
      _stroke_active = true;
    }

    void water::endPaintStroke()
    {
    ...
    bool water::follow_terrain() const
    {
      return _follow_terrain.get();
    }

    bool water::use_liquids_intersect() const
    {
      return _cursor_intersect_liquids.get();
    }
