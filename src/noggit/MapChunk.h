...
enum class ChunkTransferOverlay : int
{
  NONE = 0,
  SOURCE = 1,
  TARGET = 2
...
public:
  ...
  bool removeTextureDuplicates();
  bool removeTexturesBelowAlpha(float threshold);
  void clearTextureFlags();
  ...
  void setChunkTransferOverlay(ChunkTransferOverlay state);
  ChunkTransferOverlay chunkTransferOverlay() const;

private:
  ChunkTransferOverlay _chunk_transfer_overlay = ChunkTransferOverlay::NONE;
};
