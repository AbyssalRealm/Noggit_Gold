                            ...
                            )
{
  ...
  for (liquid_layer& layer : _layers)
  {
    ...
    {
      layer.paintLiquid(pos, radius, add, angle, orientation, lock, origin, override_height, chunk, opacity_factor, follow_terrain);
      painted = true;
    }
    else
    {
      layer.paintLiquid(pos, radius, false, angle, orientation, lock, origin, override_height, chunk, opacity_factor, follow_terrain);
    }
  }
  ...
  if (hasData(0))
  {
    ...
    layer.paintLiquid(pos, radius, true, angle, orientation, lock, origin, override_height, chunk, opacity_factor, follow_terrain);
...
void ChunkWater::clearLayers()
{
  _layers.clear();
  attributes = MH2O_Attributes{};
  _layer_count = 0;
  update_layers();
  _water_tile->tagUpdate();
}

void ChunkWater::replace_layers(std::vector<liquid_layer> const& layers
                              , MH2O_Attributes const& new_attributes
                              , glm::vec3 const& base
                              , float height_offset)
{
  _layers = layers;
  ...
  {
    layer.relocate(this, base, height_offset);
  }

  _layer_count = static_cast<int>(_layers.size());
  update_layers();
  _water_tile->tagUpdate();
}
