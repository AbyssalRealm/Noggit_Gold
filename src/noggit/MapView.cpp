...
#include <noggit/tools/ClearingTool.hpp>
...
void MapView::createGUI()
{
  ...
  _tools.emplace_back(std::make_unique<Noggit::ClearingTool>(this))->setupUi(_tool_panel_dock);
...
void MapView::doSelection (bool selectTerrainOnly, bool mouseMove)
{
  ...
  // Water mode, optionally let the cursor resolve against the actual liquid surface.
  // This stays local to cursor placement and does not alter general selection semantics.
  if (terrainMode == editing_mode::water)
  {
    auto* water_tool = dynamic_cast<Noggit::WaterTool*>(activeTool().get());
    if (water_tool && water_tool->useLiquidIntersection())
    {
      if (auto liquid_hit = _world->intersectLiquid(intersect_ray()))
      {
        if (results.empty() || liquid_hit->first < results.front().first)
        {
          auto action = NOGGIT_CUR_ACTION;
          if (!action || !action->getBlockCursor() || !_locked_cursor_mode.get())
            _cursor_pos = liquid_hit->second;
          emit rotationChanged();
          return;
        }
      }
...
void MapView::update_cursor_pos()
{
  ...
  // Water mode cursor ray check for per-frame cursor updates.
  if (terrainMode == editing_mode::water)
  {
    auto* water_tool = dynamic_cast<Noggit::WaterTool*>(activeTool().get());
    if (water_tool && water_tool->useLiquidIntersection())
