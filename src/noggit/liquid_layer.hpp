...
public:
  ...
  // Water-tool ray hit. Kept separate from the general selection variant.
  std::optional<std::pair<float, glm::vec3>> intersect(math::ray const& ray) const;

  // Chunk transfer support. These operations keep the liquid layer's private
  // masks, vertices, base position, and owning ChunkWater synchronized.
  void relocate(ChunkWater* chunk, glm::vec3 const& base, float height_offset);
