...
std::optional<std::pair<float, glm::vec3>> World::intersectLiquid(math::ray const& ray)
{
  std::optional<std::pair<float, glm::vec3>> best;
  ...
  for (auto& pair : _loaded_tiles_buffer)
  {
    ...
    TileIndex const index{static_cast<std::size_t>(pair.first.first), static_cast<std::size_t>(pair.first.second)};
    if (!mapIndex.tileLoaded(index) || mapIndex.tileAwaitingLoading(index) || !tile->finishedLoading())
      continue;

    if (auto hit = tile->Water.intersectLiquid(ray))
    {
...
bool World::clearOnChunk(MapChunk* chunk, ClearingOptions const& options)
{
  bool changed = false;
  ...
  bool const texture_operation = options.textures
                              || options.duplicate_textures
                              || options.textures_below_threshold
                              || options.texture_flags;
  if (texture_operation)
  {
    NOGGIT_CUR_ACTION->registerChunkTextureChange(chunk);

    if (options.textures)
    {
      bool const had_textures = chunk->getTextureSet()->num() > 0;
      chunk->eraseTextures();
      changed |= had_textures;
    }

    if (options.duplicate_textures)
    {
      changed |= chunk->removeTextureDuplicates();
    }

    if (options.textures_below_threshold)
    {
      changed |= chunk->removeTexturesBelowAlpha(options.alpha_threshold);
    }

    if (options.texture_flags)
    {
      bool had_flags = false;
      for (size_t i = 0; i < chunk->getTextureSet()->num(); ++i)
      {
        if (chunk->getTextureSet()->flag(i) != 0)
        {
          had_flags = true;
          break;
        }
      }
      chunk->clearTextureFlags();
      changed |= had_flags;
    }
  }

  if (options.liquids && chunk->liquid_chunk()->layer_count() > 0)
  {
    NOGGIT_CUR_ACTION->registerChunkLiquidChange(chunk);
    chunk->liquid_chunk()->clearLayers();
    changed = true;
  }
  ...
  if (options.vertex_colors && chunk->hasColors())
  {
  ...
  if (options.impassible_flag && (chunk->header_flags.value & 0x2))
  {
  ...
  if (options.holes && chunk->getHoleMask() != 0)
  {
    NOGGIT_CUR_ACTION->registerChunkHoleChange(chunk);
    chunk->setHole(chunk->getCenter(), 0.0f, true, false);
    changed = true;
  }
...
void World::clearOnChunks(glm::vec3 const& pos, float radius, ClearingOptions const& options)
{
  ZoneScoped;

  for_all_chunks_in_range(pos, radius, [&](MapChunk* chunk)
  {
    return clearOnChunk(chunk, options);
  });

  if (options.m2s || options.wmos)
  {
    std::vector<selected_object_type> objects;

    auto in_affected_chunk_range = [&](glm::vec3 const& object_pos)
    {
      float const dist = misc::dist(pos.x, pos.z, object_pos.x, object_pos.z);
      if (dist < radius)
      {
        return true;
      }

      if (dist >= radius + static_cast<float>(MAPCHUNK_RADIUS))
      {
        return false;
      }

      MapChunk* chunk = getChunkAt(object_pos);
      return chunk
          && misc::getShortestDist(pos.x, pos.z, chunk->xbase, chunk->zbase, CHUNKSIZE) <= radius;
    };

    if (options.m2s)
    {
      _model_instance_storage.for_each_m2_instance([&](ModelInstance& instance)
      {
        if (in_affected_chunk_range(instance.pos))
        {
    ...
    if (options.wmos)
    {
      _model_instance_storage.for_each_wmo_instance([&](WMOInstance& instance)
      {
        if (in_affected_chunk_range(instance.pos))
        {
          objects.emplace_back(&instance);
...
void World::clearOnTiles(glm::vec3 const& pos, float radius, ClearingOptions const& options)
{
  ZoneScoped;

  std::vector<TileIndex> target_tiles;
  ...
  for (MapTile* tile : mapIndex.tiles_in_range(pos, radius))
  {
    ...
    target_tiles.emplace_back(tile->index);
    ...
    for (size_t ty = 0; ty < 16; ++ty)
    {
      ...
      {
        tile_changed |= clearOnChunk(tile->getChunk(ty, tx), options);
      }
    }
  ...
  if ((options.m2s || options.wmos) && !target_tiles.empty())
  {
    auto belongs_to_target_tile = [&](glm::vec3 const& object_pos)
    {
      TileIndex const object_tile(object_pos);
      return std::find(target_tiles.begin(), target_tiles.end(), object_tile) != target_tiles.end();
    };
    ...
    if (options.m2s)
    {
      _model_instance_storage.for_each_m2_instance([&](ModelInstance& instance)
      {
        if (belongs_to_target_tile(instance.pos))
        {
          objects.emplace_back(&instance);
        }
      });
                       ...
                       )
{
  ...
  {
    NOGGIT_CUR_ACTION->registerChunkLiquidChange(chunk);
    chunk->liquid_chunk()->paintLiquid(pos, radius, liquid_id, add, angle, orientation, lock, origin, override_height, override_liquid_id, chunk, opacity_factor, follow_terrain);
    return true;
  });
}
