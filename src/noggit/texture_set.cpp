...
bool TextureSet::eraseUnusedTextures(float threshold)
{
  ...
  if (threshold >= 255.0f)
  {
    bool const changed = nTextures > 0;
    eraseTextures();
    return changed;
  }
  ...
  int const i_threshold = static_cast<int>(threshold);
  float const normalized_threshold = threshold / 255.0f;
  std::set<int> visible_tex;
  ...
  if (tmp_edit_values)
  {
    ...
    for (int layer = 0; layer < nTextures && visible_tex.size() < nTextures; ++layer)
    {
      for (int i = 0; i < 4096; ++i)
      {
        if (amaps[layer][i] >= normalized_threshold)
        {
          visible_tex.emplace(layer);
          break;
        }
  ...
  else
  {
    for (int i = 0; i < 4096 && visible_tex.size() < nTextures; ++i)
    {
      ...
      for (int n = 0; n < nTextures - 1; ++n)
      {
        uint8_t const a = alphamaps[n]->getAlpha(i);
        sum += a;
        if (a >= i_threshold)
        {
          visible_tex.emplace(n + 1);
      ...
      int const base_alpha = std::max(0, 255 - sum);
      if (base_alpha >= i_threshold)
      {
        visible_tex.emplace(0);
      }
    }
...
void TextureSet::clearTextureFlags()
{
  for (size_t i = 0; i < nTextures; ++i)
  {
