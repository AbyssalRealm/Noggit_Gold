...
bool MapChunk::removeTextureDuplicates()
{
  return texture_set->removeDuplicate();
}

bool MapChunk::removeTexturesBelowAlpha(float threshold)
{
  return texture_set->eraseUnusedTextures(threshold);
}

void MapChunk::clearTextureFlags()
{
  texture_set->clearTextureFlags();
}
...
void MapChunk::setChunkTransferOverlay(ChunkTransferOverlay state)
{
  if (_chunk_transfer_overlay == state)
    return;

  _chunk_transfer_overlay = state;
  registerChunkUpdate(ChunkUpdateFlags::AREA_ID);
}

ChunkTransferOverlay MapChunk::chunkTransferOverlay() const
{
  return _chunk_transfer_overlay;
}
