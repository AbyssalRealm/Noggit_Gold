...
namespace Noggit
{
    ...
    void WaterTool::onTick(float deltaTime, TickParameters const& params)
    {
        ...
        if (params.displayMode == display_mode::in_3D && !params.underMap)
        {
            if (params.mod_shift_down)
            {
                ...
                {
                    _guiWater->beginPaintStroke(mv->cursorPosition());
    ...
    void WaterTool::onMouseRelease(MouseReleaseParameters const& params)
    {
        if (params.button == Qt::MouseButton::LeftButton && _paintStrokeActive)
        {
            _guiWater->endPaintStroke();
            _paintStrokeActive = false;
        }
    }
    bool WaterTool::useLiquidIntersection() const
    {
        return _guiWater && _guiWater->use_liquids_intersect();
    }
