...
std::optional<std::pair<float, glm::vec3>> TileWater::intersectLiquid(math::ray const& ray)
{
  if (!hasData())
    return std::nullopt;
  ...
  for (int z = 0; z < 16; ++z)
  {
    for (int x = 0; x < 16; ++x)
    {
      ChunkWater* water_chunk = chunks[z][x].get();
      if (!water_chunk || water_chunk->layer_count() == 0)
        continue;

      for (auto const& layer : *water_chunk->getLayers())
      {
        if (auto hit = layer.intersect(ray))
        {
