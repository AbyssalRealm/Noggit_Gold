...
void Noggit::Action::undo(bool redo)
{
  ...
  if (_flags & ActionFlags::eCHUNKS_WATER)
  {
    for (auto& pair : redo ? _chunk_liquid_post : _chunk_liquid_pre)
    {
      ...
      liquid_chunk->replace_layers(pair.second.layers, pair.second.attributes,
                                   {pair.first->xbase, pair.first->ybase, pair.first->zbase}, 0.f);
    }
  }
  ...
  if (_flags & ActionFlags::eCHUNKS_WATER)
  {
    ...
    for (int i = 0; i < _chunk_liquid_pre.size(); ++i)
    {
      ...
      post.second = {*pre.first->liquid_chunk()->getLayers(),
                     pre.first->liquid_chunk()->getAttributes()};
    }
  }
  if (_flags & ActionFlags::eCHUNKS_SOUND_EMITTERS)
  {
    ...
    for (std::size_t i = 0; i < _chunk_sound_emitters_pre.size(); ++i)
    {
      ...
      post = {pre.first, pre.first->sound_emitters};
    }
  }
  if (_flags & ActionFlags::eVERTEX_SELECTION)
  {
...
void Noggit::Action::registerChunkLiquidChange(MapChunk* chunk)
{
  ...
  _chunk_liquid_pre.emplace_back(chunk,
    ChunkLiquidChangeCache{*chunk->liquid_chunk()->getLayers(),
                           chunk->liquid_chunk()->getAttributes()});
}
...
void Noggit::Action::registerChunkSoundEmitterChange(MapChunk* chunk)
{
  ...
  for (auto const& pair : _chunk_sound_emitters_pre)
    if (pair.first == chunk)
      return;
...
void Noggit::Action::registerAllChunkChanges(MapChunk* chunk)
{
  ...
  registerChunkSoundEmitterChange(chunk);
}
