...
)
{
    ...
    || _requires_ground_effect_color_recalc || _require_geffect_active_texture_update))
  {
    ...
    for (int i = 0; i < 256; ++i)
    {
      ...
      _chunk_instance_data[i].AreaIDColor_Pad2_DrawSelection[1]
        = chunk->chunkTransferOverlay() == ChunkTransferOverlay::SOURCE;
      _chunk_instance_data[i].AreaIDColor_Pad2_DrawSelection[2]
        = chunk->chunkTransferOverlay() == ChunkTransferOverlay::TARGET;
...
void TileRender::initChunkData(MapChunk* chunk)
{
  ...
  chunk_render_instance.AreaIDColor_Pad2_DrawSelection[1]
    = chunk->chunkTransferOverlay() == ChunkTransferOverlay::SOURCE;
  chunk_render_instance.AreaIDColor_Pad2_DrawSelection[2]
    = chunk->chunkTransferOverlay() == ChunkTransferOverlay::TARGET;
