        ...
        if (add)
        {
          for (int index : {id, id + 1, id + 9, id + 10})
          {
            ...
            if (no_subchunk || (in_range && override_height))
            {
              if (follow_terrain)
              {
                int const vertex_x = index % 9;
                int const vertex_z = index / 9;
                int const terrain_index = vertex_z * 17 + vertex_x;
                _vertices[index].position.y = chunk->mVertices[terrain_index].y + 1.0f;
              }
              else
              {
...
std::optional<std::pair<float, glm::vec3>> liquid_layer::intersect(math::ray const& ray) const
{
  ...
  for (int z = 0; z < 8; ++z)
  {
    for (int x = 0; x < 8; ++x)
    {
      ...
      int const id0 = z * 9 + x;
      int const id1 = id0 + 1;
      int const id2 = (z + 1) * 9 + x;
      int const id3 = id2 + 1;

      auto consider = [&](int a, int b, int c)
      {
        if (auto const distance = ray.intersect_triangle(_vertices[a].position, _vertices[b].position, _vertices[c].position))
        {
          if (!best || *distance < best->first)
            best = std::make_pair(*distance, ray.position(*distance));
        }
      };

      consider(id0, id1, id2);
      consider(id2, id3, id1);
    }
  }
...
void liquid_layer::relocate(ChunkWater* chunk, glm::vec3 const& base, float height_offset)
{
  glm::vec3 const delta(base.x - pos.x, height_offset, base.z - pos.z);
...
void liquid_layer::rotate_90_degrees()
{
  auto const original_vertices = _vertices;
  std::uint64_t const original_subchunks = _subchunks;

  for (int x = 0; x < 9; ++x)
  {
    for (int z = 0; z < 9; ++z)
    {
      int const old_index = z * 9 + x;
      int const new_x = z;
      int const new_z = 8 - x;
      int const new_index = new_z * 9 + new_x;

      _vertices[new_index] = original_vertices[old_index];
      _vertices[new_index].position.x = pos.x + new_x * UNITSIZE;
      _vertices[new_index].position.z = pos.z + new_z * UNITSIZE;
    }
  }
  ...
  for (int x = 0; x < 8; ++x)
  {
    for (int z = 0; z < 8; ++z)
    {
      if ((original_subchunks >> (z * 8 + x)) & std::uint64_t(1))
      {
        int const new_x = z;
        int const new_z = 7 - x;
        _subchunks |= std::uint64_t(1) << (new_z * 8 + new_x);
      }
    }
  }
...
void liquid_layer::mirror(bool horizontal)
{
  auto const original_vertices = _vertices;
  std::uint64_t const original_subchunks = _subchunks;
